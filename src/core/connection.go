...
var WsConnMap = make(map[string]*ConnectionHandler) // 全局连接池
var WsConnMapLock sync.RWMutex

// 全局缓存: mac -> session_id
var MacSessionMap = make(map[string]string)
var MacSessionMapLock sync.RWMutex
...
// GetTalkRound returns the current talk round
func (h *ConnectionHandler) GetTalkRound() int {
	return h.talkRound
}
...
func (h *ConnectionHandler) GetClientID() string {
	return h.clientId
}

// GetHeaders returns the headers map
func (h *ConnectionHandler) GetHeaders() map[string]string {
	return h.headers
}
...
func NewConnectionHandler(
	...
	// 从 URL 参数中提取 client-id
	if clientID := req.URL.Query().Get("client-id"); clientID != "" {
	...
	WsConnMapLock.Lock()
	if _, exists := WsConnMap[handler.sessionID]; !exists {
		WsConnMap[handler.sessionID] = handler
	}
	WsConnMapLock.Unlock()

	if handler.clientId != "" {
		MacSessionMapLock.Lock()
		MacSessionMap[handler.clientId] = handler.sessionID
		MacSessionMapLock.Unlock()
		fmt.Println("缓存client-id-session:", handler.clientId, handler.sessionID)
	}
...
func (h *ConnectionHandler) Close() {
	h.closeOnce.Do(func() {
		...
		// 移除连接
		WsConnMapLock.Lock()
		delete(WsConnMap, h.sessionID)
		WsConnMapLock.Unlock()

		if h.deviceID != "" {
			MacSessionMapLock.Lock()
			delete(MacSessionMap, h.deviceID)
			MacSessionMapLock.Unlock()
		}
	})
}
